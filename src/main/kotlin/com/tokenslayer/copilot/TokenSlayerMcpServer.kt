...
class TokenSlayerMcpServer {
    ...
    private fun buildInitializeResult(): JsonObject = JsonObject().apply {
    ...
    private fun buildToolsList(): JsonObject = JsonObject().apply {
            ...
            add("inputSchema", JsonObject().apply {
                ...
                add("properties", JsonObject().apply {
                    add("filePath", JsonObject().apply {
                        ...
                        addProperty("description", "Absolute path to the file to summarize. Omit to use the currently active file.")
                    })
                    add("verbosity", JsonObject().apply {
                        addProperty("type", "string")
    ...
    private fun executeTool(toolName: String, args: JsonObject): JsonObject {
        if (toolName != TOOL_NAME) {
            return buildToolError("Unknown tool: $toolName")
        }

        val verbosity = args.get("verbosity")?.asString?.let { Verbosity.from(it) }
            ?: TokenSlayerSettings.getInstance().verbosity

        val project = ProjectManager.getInstance().openProjects.firstOrNull()
            ?: return buildToolError("No open project found")

        val filePath = args.get("filePath")?.asString?.takeIf { it.isNotBlank() }
            ?: run {
                // Use active editor file
                var active: String? = null
                ApplicationManager.getApplication().invokeAndWait {
                    active = FileEditorManager.getInstance(project)
        ...
        val vFile = com.intellij.openapi.vfs.LocalFileSystem.getInstance().findFileByPath(filePath)
            ?: return buildToolError("File not found: $filePath")

        val result = TokenSlayerService.getInstance().analyzeFile(vFile, project)
        ...
        return buildToolSuccess(result.skeleton, filePath, fromCache = false,
            originalTokens = result.originalTokens, skeletonTokens = result.skeletonTokens)
    }

    // ── JSON-RPC Helpers ──────────────────────────────────────────────────────

    private fun buildResult(id: com.google.gson.JsonElement?, result: JsonObject): JsonObject =
        JsonObject().apply {
    ...
    private fun buildError(code: Int, message: String): JsonObject =
        JsonObject().apply {
            addProperty("jsonrpc", "2.0")
            addProperty("id", 1)
        ...
        content.add(JsonObject().apply {
            ...
            addProperty("text", buildString {
                ...
                if (originalTokens > 0) {
                    ...
                    appendLine("⚡ Token reduction: ${TokenEstimator.format(originalTokens)} → ${TokenEstimator.format(skeletonTokens)} ($pct% saved)")
                }
    ...
    private fun buildToolError(message: String): JsonObject = JsonObject().apply {
        val content = com.google.gson.JsonArray()
        content.add(JsonObject().apply {
            addProperty("type", "text")
