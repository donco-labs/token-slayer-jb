...
object SecretsDetector {
    ...
    // ── Content patterns ────────────────────────────────────────────────────
    private val CONTENT_PATTERNS: List<ContentPattern> = listOf(
        // API Keys & Tokens
        ContentPattern(Regex("""(?:api[_\-]?key|apikey)\s*[:=]\s*['"][A-Za-z0-9_\-]{16,}""", RegexOption.IGNORE_CASE), "API key", Severity.HIGH),
        ContentPattern(Regex("""(?:secret[_\-]?key|secretkey)\s*[:=]\s*['"][A-Za-z0-9_\-/+=]{16,}""", RegexOption.IGNORE_CASE), "Secret key", Severity.HIGH),
        ContentPattern(Regex("""(?:access[_\-]?token|accesstoken)\s*[:=]\s*['"][A-Za-z0-9_\-]{16,}""", RegexOption.IGNORE_CASE), "Access token", Severity.HIGH),
        ContentPattern(Regex("""(?:auth[_\-]?token|authtoken)\s*[:=]\s*['"][A-Za-z0-9_\-]{16,}""", RegexOption.IGNORE_CASE), "Auth token", Severity.HIGH),
        ...
        ContentPattern(Regex("""(?:aws[_\-]?secret|aws_secret_access_key)\s*[:=]\s*['"][A-Za-z0-9/+=]{30,}""", RegexOption.IGNORE_CASE), "AWS Secret Key", Severity.HIGH),
        ...
        ContentPattern(Regex("""(?:mongodb|postgres(?:ql)?|mysql|redis|amqp|mssql|mariadb|couchdb|cassandra)://[^\s'"]{10,}""", RegexOption.IGNORE_CASE), "Database connection string", Severity.HIGH),
        ContentPattern(Regex("""(?:password|passwd|pwd)\s*[:=]\s*['"][^'"]{6,}""", RegexOption.IGNORE_CASE), "Password", Severity.HIGH),
        ContentPattern(Regex("""(?:db[_\-]?password|database[_\-]?password)\s*[:=]\s*['"][^'"]{4,}""", RegexOption.IGNORE_CASE), "Database password", Severity.HIGH),
        ...
        // JWT
        ContentPattern(Regex("""(?:jwt[_\-]?secret|jwt_secret_key)\s*[:=]\s*['"][^'"]{8,}""", RegexOption.IGNORE_CASE), "JWT secret", Severity.HIGH),

        // SSH
        ContentPattern(Regex("""(?:ssh[_\-]?pass|sshpass)\s*[:=]\s*['"][^'"]{4,}""", RegexOption.IGNORE_CASE), "SSH password", Severity.HIGH),

        // Generic environment secrets
        ContentPattern(Regex("""(?:SECRET|TOKEN|CREDENTIAL|AUTH)[A-Z_]*\s*=\s*['"]?[A-Za-z0-9_\-/+=]{20,}""", RegexOption.IGNORE_CASE), "Environment secret", Severity.MEDIUM),
    )

    // ── Filename patterns ────────────────────────────────────────────────────
    private val SENSITIVE_FILENAMES: List<FilenamePattern> = listOf(
     ...
     */
    fun scan(filePath: String, content: String): SecretsScanResult {
        val reasons = mutableListOf<String>()
        var highestSeverity = Severity.LOW

        fun raiseSeverity(s: Severity) {
            if (s == Severity.HIGH) highestSeverity = Severity.HIGH
            else if (s == Severity.MEDIUM && highestSeverity != Severity.HIGH) highestSeverity = Severity.MEDIUM
        }
